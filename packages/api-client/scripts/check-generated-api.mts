// Verifies the committed orval client in src/generated is exactly what
// regenerating from server/generated/openapi.json produces. Regenerates into
// a throwaway directory inside packages/api-client/ (so prettier config
// resolution matches) and compares trees without touching the committed
// files.
...
const tmpRoot = resolve(packageRoot, "api-check-tmp");
const specPath =
  process.env.POPCHARTS_API_SPEC ??
  resolve(packageRoot, "../../server/generated/openapi.json");
...
`
    ...
    execFileSync(
      "pnpm",
      ["exec", "orval", "--config", relative(packageRoot, configPath)],
      {
        cwd: packageRoot,
    ...
    if (problems.length > 0) {
      console.error(
        `Committed API client at ${relative(packageRoot, committedRoot)} is out of date ` +
