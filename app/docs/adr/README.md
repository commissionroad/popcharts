...
This directory records product and engineering decisions for the Pop Charts
frontend app.

ADRs should be short enough to read before changing code, but specific enough
to stop the same arguments from being reopened every week. These initial
decisions are intentionally about the ground rules for implementation inside
this top-level `app/` folder.
