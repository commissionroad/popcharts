...
| `PriceCurve`       | `app/src/components/charts/price-curve.tsx`   | None                                    | YES/NO price-history chart           | One phase-blind `points` list (`{at?, yesCents, noCents}`, repo ADR 0025), optional `graduatedAt` annotation and `yesLabel`/`noLabel`; trailing-window pills (1H-1M, ALL), quarter gridlines with axis values, crosshair hover readout; the annotation places the graduation rule, shaded venue region, and complete-set readout | Market detail feature                                                                    |
...
| `SmallMetric`      | `app/src/components/ui/small-metric.tsx`      | None                                    | Compact labelled stat figure         | `label`, `value`; uppercase mono label over a bold display value                                                                                                                                                                                                                                                                 | Market detail (live stats island + settled summaries)                                    |
