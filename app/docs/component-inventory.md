...
| `PriceCurve`       | `app/src/components/charts/price-curve.tsx`   | None                                    | YES/NO price-history chart           | `points`, optional `postgradPoints`/`graduatedAt`/`yesLabel`/`noLabel`; trailing-window pills (1H-1M, ALL), quarter gridlines with axis values, crosshair hover readout; graduation rule plus shaded venue region and complete-set readout when the postgrad inputs are supplied | Market detail feature                                                                    |
