...
| `PriceCurve` | `app/src/components/charts/price-curve.tsx` | None | Virtual LMSR price-path SVG | `path`, `side`; yes/no color state | Market detail feature |
| `Button` | `app/src/components/ui/button.tsx` | `designkit/components/Button` | Brand button/link primitive | Variants `primary`, `secondary`, `ghost`; sizes `sm`, `md`, `lg`; `href`, `leftIcon`, `rightIcon`, `glow`, disabled state | App nav, error pages, create flow, market detail dev actions, graduation, receipt ticket |
...
| `GraduationBar` | `app/src/components/ui/graduation-bar.tsx` | `designkit/components/GraduationBar` | Matched-liquidity progress indicator | `matchedUsd`, `targetUsd`, optional `height`, optional caption | Market cards, market detail, graduation clearing |
...
| `OutcomeButton` | `app/src/components/ui/outcome-button.tsx` | `designkit/components/OutcomeButton` | YES/NO price action tile | `side`, `priceCents`, optional `href`, `selected`, `sub` | Market cards |
| `SegmentedControl` | `app/src/components/ui/segmented-control.tsx` | `designkit/components/SegmentedControl` | Tokenized segmented selector | `options`, `value`, `onChange`, optional `size`, `full`, `accentBy`, `className` | Discovery filters, receipt ticket |
