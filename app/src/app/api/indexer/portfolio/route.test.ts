...
describe("GET /api/indexer/portfolio", () => {
  it("fails with 500 when no indexer API is configured", async () => {
    const response = await GET(portfolioRequest());

    expect(response.status).toBe(500);
    expect(await errorOf(response)).toBe(
      "POPCHARTS_INDEXER_API_URL is required to read portfolios."
    );
  });

  it("requires both chainId and owner query parameters", async () => {
    vi.stubEnv("POPCHARTS_INDEXER_API_URL", "http://indexer:3011");

    const missingOwner = await GET(
      new Request("http://app.local/api/indexer/portfolio?chainId=31337")
    );
    const missingChain = await GET(
      new Request(`http://app.local/api/indexer/portfolio?owner=${OWNER}`)
    );

    expect(missingOwner.status).toBe(400);
    expect(missingChain.status).toBe(400);
    expect(await errorOf(missingOwner)).toBe(
      "chainId and owner query parameters are required."
    );
  });

  it("proxies the indexer's portfolio for the requested owner", async () => {
    vi.stubEnv("POPCHARTS_INDEXER_API_URL", "http://indexer:3011");
    const portfolio = { chainId: 31337, owner: OWNER };
    const fetcher = stubUpstream(
      new Response(JSON.stringify(portfolio), { status: 200 })
    );
    ...
    expect(String(fetcher.mock.calls[0]?.[0])).toBe(
      `http://indexer:3011/portfolio/31337?owner=${OWNER}`
    );
  });
  ...
  it("returns 404 when the indexer does not know the portfolio", async () => {
    vi.stubEnv("POPCHARTS_INDEXER_API_URL", "http://indexer:3011");
    ...
    expect(await errorOf(response)).toBe("Portfolio not found.");
  });
  ...
  it("maps upstream failures to 502 with generic copy (no raw indexer message)", async () => {
    ...
    expect(await errorOf(response)).toBe("Portfolio request failed.");
  });
...
function portfolioRequest() {
  return new Request(
    `http://app.local/api/indexer/portfolio?chainId=31337&owner=${OWNER}`
  );
}
