...
| ------------------------------------ | ------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AI_RESOLUTION_PROVIDER`             | `ollama`                 | `ollama`, `anthropic`, `claude-cli`, `codex-cli`, or `heuristic`. Only `ollama` and `anthropic` capture evidence, and `resolver.ts` needs an evidence item to grade a decided outcome — the CLI providers park every case at `manual_review`. `heuristic` always abstains.                                                                     |
| `AI_RESOLUTION_PORT`                 | `3004`                   | Also the runner's default `--service-url` port.                                                                                                                                                                                                                                                                                                |
| `AI_RESOLUTION_TIMEOUT_MS`           | `300000`                 | Now generous by default. It was `8000`, which fail-safed every model-backed run to `manual_review`/`service_error` and cost a full eval run before anyone noticed; the default was raised because no model-backed provider fits in 8 s. The runner's own request timeout (360 s) must stay above it. Lower it only for the heuristic provider. |
