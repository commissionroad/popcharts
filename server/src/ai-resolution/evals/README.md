...
| ------------------------------------ | ------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AI_RESOLUTION_PROVIDER`             | `codex-cli`              | `codex-cli`, `claude-cli`, `ollama`, `anthropic`, or `heuristic` (heuristic always abstains on these cases — pointless for evals).                                                                                                                                                  |
| `AI_RESOLUTION_PORT`                 | `3004`                   | Also the runner's default `--service-url` port.                                                                                                                                                                                                                                     |
| `AI_RESOLUTION_TIMEOUT_MS`           | `300000`                 | Now generous by default. It was `8000`, which fail-safed every model-backed run to `manual_review`/`service_error` and cost a full eval run before anyone noticed; the default was raised when a CLI provider became the service default. Lower it only for the heuristic provider. |
