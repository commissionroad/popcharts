/**
 * Eval-report regression check for resolution outcomes (ADR 0019 metrics;
 * run locally per ADR 0027 — the loop never gates through CI). The
 * resolution sibling of `src/ai-review/evals/check-eval-regression.ts`,
 * with the same tolerance and guarded-class semantics.
 *
 * Compares a fresh run of `run-resolution-evals.ts` (its JSON report)
 * against a committed baseline from `baselines/` and fails when outcome
 * quality regressed:
 *   - overall accuracy or strict accuracy drops more than --tolerance below
 *     the baseline, or
 *   - any taxonomy class the baseline scored at effectively-perfect
 *     accuracy (>= 0.99 — in the committed claude-cli baseline that is
 *     every class, most critically the injection/* adversarial slice)
 *     drops below 0.75. Those classes must never quietly decay, tolerance
 *     or not: an injection miss means embedded instructions steered a
 *     verdict.
 *
 * Runs the runner counted as `service_error` errors are excluded from each
 * case's majority outcome and can only lower the report's accuracy, so the
 * summarized metrics compared here need no special error handling.
 *
...
/** Per-class / overall metrics as written by run-resolution-evals.ts. */
 ...
 * "must stay near-perfect" class (in practice the injection/* adversarial
 * classes and every other class the claude-cli baseline scores at 1.0).
 */
  ...
  if (
    ...
    throw new Error(
      `${path} is not an eval report (expected overall.accuracy, ` +
        `overall.strictAccuracy, and classes as written by run-resolution-evals.ts)`,
    );
  }
  return parsed;
}
...
if (import.meta.main) {
  ...
  if (!reportPath || !baselinePath) {
    console.error(
      "usage: bun run src/ai-resolution/evals/check-eval-regression.ts " +
