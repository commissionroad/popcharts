...
export const MARKET_REVIEW_POLICY = [
  ...
  "Check resolution coherence before approving: mentally apply the resolution criteria to a few plausible source readings and confirm it returns the SAME yes/no as the plain-language question. When a realizable reading makes the criteria resolve OPPOSITE to the question — most often an aggregate/quantifier mismatch, where the question asks about an aggregate (the maximum, minimum, total, average, or count) while the criteria fire on ANY single reading, or the reverse — raise the hard flag \"incoherent_resolution\" and reject. Naming the maximum but resolving YES on 'any reading below the threshold' (or the minimum but 'any reading above it') is contradictory, because one reading on the wrong side already flips the true aggregate. In reasons, name the contradiction and tell the creator which aggregate the criteria must measure so they can fix it. This is a mis-specification of an otherwise-answerable question, so it is a reject — not the manual_review used for a merely missing tie, timezone, or revision clause.",
...
export const MARKET_REVIEW_EXAMPLES = [
  ...
  "Worked example 3 — resolution incoherence (aggregate vs any). Question: 'Will the maximum temperature be lower than 75°F during the window?' Criteria: 'Resolve YES if any reading is strictly lower than 75°F.' Verdict: reject. 'Maximum lower than 75°F' is true only if EVERY reading stays below 75°F, but the criteria fire on ANY single reading — so a window with a 77°F reading and a 73°F reading resolves YES even though the true maximum (77°F) is not below 75°F, the exact opposite of the question. Fix: resolve from the aggregate the question names, e.g. 'YES only if no reading is at or above 75°F'. The mirror case is coherent and must not be rejected: 'maximum HIGHER than X' matches 'any reading above X', since one reading above X already makes the maximum above X.",
].join("\n");
