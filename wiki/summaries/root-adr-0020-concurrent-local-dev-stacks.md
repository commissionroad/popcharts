...
- **Phase 4 — stack-aware create-market (#248).** `resolveTargetStack` selects
  ...
  **Correction (2026-07-25):** this resolved the target for the script's own
  ...
  `scripts/shared/localStack/protocolChainEnv.ts` and shared with the Phase 5
  launcher. The same sweep found two more instances of the split: `--api-url`
...
- **Phase 5 — cross-workspace targeting scripts (#260).** The `bun`/`hardhat`
  ...
  2026-07-25). Correct same-worktree; cross-slot deploy from one checkout still
