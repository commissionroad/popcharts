...
## [2026-07-17] ingest | repo ADR 0021 — slice 1 (server spine) landed
Pages: ~summaries/root-adr-0021-live-market-updates.md, ~index.md
Notes: Server-side change_feed outbox spine built under server/src/live/ — generic to_jsonb(NEW) capture trigger (idempotent ensure-script wired into local-dev after db:push; prod runs it after db:migrate per ADR 0015), poll-based relay + in-process hub (lookback/dedup for the sequence-visibility gap, ref-counted autostart so idle instances don't poll), and GET /events SSE route with Last-Event-ID replay. Market-keyed source set only; pool/token-keyed routing (ticks/orders/transfers) and the review-job UPDATE progress deferred to the surface slices. Verified: 412 server tests pass, typecheck/lint/format clean, OpenAPI valid (SSE route hidden). Slices 2–7 open.

## [2026-07-20] ingest | repo ADR 0021 — PR #249 architecture correction
Pages: ~summaries/root-adr-0021-live-market-updates.md, ~entities/indexer.md, ~entities/server-workspace.md, ~index.md
Notes: Architecture review disproved exact `bigserial`/`Last-Event-ID` replay with two checked-in red regressions plus a real two-connection PostgreSQL probe: a lower sequence id can commit after a higher cursor and become unreplayable. Authoritative blockchain-derived rows remain intact; only the invalidation is missed. Slice 1 reopened. Revised target keeps SSE + Postgres but uses an explicit transactional semantic outbox, subscribe-then-refetch reconnect correctness, domain sequences for incremental payloads, bounded/coalesced queues, migration-owned DB behavior, and mandatory 24–48h age-based pruning. Never delete on consumption because clients/API instances have independent cursors; PR #249 currently has no deletion path and grows forever.
