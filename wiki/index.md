...
- [ADR 0014](summaries/root-adr-0014-full-lifecycle-e2e-testing.md) — the every-terminal-state e2e suite; acceptance gate for M1–M4 (delivery re-homed into ADR 0017 Track C: service/chain layer + five UI journeys; scenarios open)
...
- [ADR 0022](summaries/root-adr-0022-review-first-market-creation.md) — Proposed: invert market creation to review-first — questions live as off-chain editable Drafts reviewed before any chain write; on approval the creator publishes via a gated `createMarket` (full-params authorizer signature, on-chain single-use nonce, trusted-creator bypass) so markets are born Active and the fee is paid at publish not submit; retires the on-chain review path, adds templates/clone, Privy-auth drafts, and a real-markets-only board; anti-spam = rate limiting (accepted risk); red-teamed (8 phases open)
