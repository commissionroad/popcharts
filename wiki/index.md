...
- [ADR 0014](summaries/root-adr-0014-full-lifecycle-e2e-testing.md) — the every-terminal-state e2e suite; acceptance gate for M1–M4 (delivery re-homed into ADR 0017 Track C: service/chain layer + five UI journeys; all eight service/chain paths landed 2026-07-20/21 = ADR 0017 C3 complete; five UI journeys (C4) open)
...
- [ADR 0021](summaries/root-adr-0021-live-market-updates.md) — Accepted (server spine built 2026-07-22): make the app feel live via server-signalled, client-refetched updates over SSE fed by a durable `change_feed` outbox (written atomically with each indexed event via explicit `recordLiveChange` seams, not a DB trigger); DB/REST stays the single source of truth, indexer stays pure, no message broker; NOTIFY demoted to an optional poll-first doorbell (slice 1 built as a 7-PR stack, slices 2–7 open)
