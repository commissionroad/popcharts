...
- [ADR 0021](summaries/root-adr-0021-live-market-updates.md) — make the app feel live via server-signalled, client-refetched updates over SSE fed by a durable `change_feed` outbox written by explicit in-transaction TypeScript seams (no DB trigger); DB/REST stays the single source of truth, indexer stays pure, no message broker; push is best-effort with subscribe-then-refetch reconnect correctness (slice 1 server spine landed; surface slices 2–7 open)
