...
- [ADR 0022](summaries/root-adr-0022-review-first-market-creation.md) — Accepted, half built: invert market creation to review-first — questions live as off-chain editable Drafts reviewed before any chain write; on approval the creator publishes via a gated `createMarket` (full-params authorizer signature, on-chain single-use nonce, trusted-creator bypass) so markets are born Active and the creation fee is paid at publish not submit; retires the on-chain review path, adds templates/clone, Privy-auth drafts, and a real-markets-only board; anti-spam = **prepaid review credit** in a separate native-USDC vault, off-chain-metered (drafts, draft review incl. a third `changes_requested` outcome, the bond, and templates landed 2026-08-03 as PRs #412–#417/#419; **the contract gate is still open, so `createMarket` remains ungated and publish force-approves over it** — P4 keystone, P5/P6/P8 open). **P3's refundable bond was withdrawn 2026-08-04** and replaced by non-refundable credit at one configurable per-review rate (`depositFor`, no withdrawal, no settlement) — **P3a delivered 2026-08-04**, lifecycle lane runs metered. **P4 (the contract gate) and P5 (retiring the on-chain review path and the legacy create surface) both delivered 2026-08-04** — markets are now born Active through an authorized publish; P6 (metadata from the event) and P8 (server-side discovery filters) remain open
