...
- [ADR 0021](summaries/root-adr-0021-live-market-updates.md) — Accepted (server spine + client transport built 2026-07-22..23): make the app feel live via server-signalled, client-refetched updates over SSE fed by a durable `change_feed` outbox (written atomically with each indexed event via explicit `recordLiveChange` seams, not a DB trigger); DB/REST stays the single source of truth, indexer stays pure, no message broker; NOTIFY demoted to an optional poll-first doorbell; the client hook hands each signal to a **caller-supplied callback** (no React Query coupling) (slices 1–2 built, slices 3–7 open — no live UI yet)
