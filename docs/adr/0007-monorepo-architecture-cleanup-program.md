...
| Date | Item | PR | Notes |
| ---- | ---- | -- | ----- |
| 2026-07-06 | B3 | TBD | Chose the new-module placement: the indexer factory moved verbatim to `server/src/blockchain/client.ts` (shared home, since importing from `indexer/` into `api/` crosses subsystem boundaries), which now also exports `createReadOnlyClient()` and `createWalletClient(account)` HTTP factories; migrated the ad-hoc clients in `api/services/markets.ts`, `api/services/dev-market-close.ts`, and `ai-review-runner/chain-review.ts` onto them with no config or behavior changes. |
