...
| Date | Item | PR | Notes |
| ---- | ---- | -- | ----- |
| 2026-07-06 | E1 | TBD | Split `app/src/integrations/wallet/wallet-provider.tsx` into `privy-wallet-provider.tsx` (Privy provider), `local-wallet-provider.tsx` (local wagmi fallback), and `wallet-utilities.ts` (shared helpers, pending-action/summary types, disabled-value constants), keeping `wallet-provider.tsx` as the context definition + orchestrator with its public exports (`WalletProviders`, `useWalletAccount`, `WalletAccountValue`, `WalletConnectionSummary`) unchanged — code moved verbatim, no importer changed. |
