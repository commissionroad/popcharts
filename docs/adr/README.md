...
| [0021](0021-live-market-updates.md) | Proposed | Make the app feel live: server-signalled, client-refetched updates over SSE, fed by a durable `change_feed` outbox written by explicit in-transaction TypeScript seams (no DB trigger). DB/REST stays the single source of truth; push is best-effort with subscribe-then-refetch reconnect correctness; no message broker. |
