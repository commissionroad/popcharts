...
| [0022](0022-review-first-market-creation.md) | Proposed | Invert market creation to review-first: questions live as off-chain editable Drafts, on-chain `createMarket` is gated by an authorizer signature (trusted-creator bypass) so markets are born Active, the fee is paid at publish (not submit), and creators get drafts/templates + a creator surface while the public board shows real markets only. |
