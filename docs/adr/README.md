...
| [0026](0026-durable-resolution-intent.md) | Proposed | Make the resolution runner's reasoning durable before it acts: insert the `market_resolutions` row `pending`, then `proposeResolution`, then confirm it, with a sweep that confirms or abandons anything left behind. Readers filter on `commit_state`; the enqueue predicate must count only confirmed rows or a failed propose strands its market. |
...
which extends 0024's runner) are likewise standalone tracked
programs.
